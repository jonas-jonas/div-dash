...
func TestUser(t *testing.T) {
	...
	t.Run("GET /user with valid id", func(t *testing.T) {
		...
		assert.JSONEq(t, `{"id": 0, "email": "test@test.de"}`, w.Body.String())
	})
	...
	t.Run("POST /user", func(t *testing.T) {
		...
		assert.Equal(t, 200, w.Code)
		assert.JSONEq(t, `{"id": 1, "email": "test@email.com"}`, w.Body.String())
	})
	...
	t.Run("POST /user with existing email", func(t *testing.T) {
		...
		assert.JSONEq(t, `{"message": "A user with email 'test@test.de' already exists"}`, w.Body.String())
