...
func Prepare(ctx context.Context, db DBTX) (*Queries, error) {
	...
	if q.getExchangesOfSymbolStmt, err = db.PrepareContext(ctx, getExchangesOfSymbol); err != nil {
		return nil, fmt.Errorf("error preparing query GetExchangesOfSymbol: %w", err)
	}
...
func (q *Queries) Close() error {
	...
	if q.getExchangesOfSymbolStmt != nil {
		if cerr := q.getExchangesOfSymbolStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing getExchangesOfSymbolStmt: %w", cerr)
		}
	}
	if q.getJobStmt != nil {
...
type Queries struct {
	...
	getExchangesOfSymbolStmt         *sql.Stmt
	getJobStmt                       *sql.Stmt
...
func (q *Queries) WithTx(tx *sql.Tx) *Queries {
	return &Queries{
		...
		getExchangesOfSymbolStmt:         q.getExchangesOfSymbolStmt,
